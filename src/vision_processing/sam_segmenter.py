...
class SAMSegmenter:
    ...
    def extract_part_image(
        ...
        part_id: str,
        output_dir: str,
        cv_detection: Optional[Any] = None,
        bbox_hint: Optional[Tuple[int, int, int, int]] = None  # Legacy support
    ) -> Optional[str]:
        """
        ...
        Args:
            ...
            cv_detection: Optional CV DetectedPart object (preferred)
            bbox_hint: Optional bounding box hint (x1, y1, x2, y2) - DEPRECATED
        ...
        # Priority: cv_detection > bbox_hint > auto-segmentation

        # Option 1: Use CV detection (PREFERRED)
        if cv_detection:
            logger.info(f"Extracting part {part_id} using CV detection at {cv_detection.center}")

            # Use center point for SAM segmentation (most accurate)
            x, y = cv_detection.center
            segments = self.segment_page(page_path, points=[(x, y)])

            if segments:
                # SAM returns segments sorted by relevance to the point
                ...
                # Fallback: use CV bbox directly
                logger.warning(f"SAM segmentation failed, using CV bbox directly")
                component = self.extract_component(
                    image_path=page_path,
                    bbox=cv_detection.bbox,
                    output_path=output_path
                )
                return output_path if component else None

        # Option 2: Legacy bbox hint support
        elif bbox_hint:
            logger.info(f"Extracting part {part_id} using bbox hint: {bbox_hint}")
            component = self.extract_component(
                image_path=page_path,
                bbox=bbox_hint,
                output_path=output_path
        ...
        # Option 3: Auto-segmentation fallback
            ...
            logger.info(f"No CV detection or bbox for part {part_id}, using auto-segmentation fallback")
    ...
    def extract_subassembly_image(
        ...
        # Priority: cv_detection > bbox_hint > heuristic

        # Option 1: Use CV detection (PREFERRED)
        if cv_detection:
            logger.info(f"Extracting subassembly {subassembly_id} using CV detection at {cv_detection.center}")

            # Use center point for SAM segmentation
            x, y = cv_detection.center
            segments = self.segment_page(page_path, points=[(x, y)])
        ...
        # Option 2: Legacy bbox hint support
            ...
            logger.info(f"Extracting subassembly {subassembly_id} using bbox hint: {bbox_hint}")
            component = self.extract_component(
                image_path=page_path,
                bbox=bbox_hint,
                output_path=output_path
        ...
        # Option 3: Heuristic fallback
            ...
            logger.warning(f"No CV detection or bbox for subassembly {subassembly_id}, using heuristic fallback")
