...
async def unified_query(
    ...
    """
    Unified query endpoint using intelligent query routing.

    This endpoint automatically determines query intent and routes to the appropriate handler:
    - "What's next?" → NextStepHandler (graph-based)
    - "What parts do I need?" → PartsHandler (graph + RAG)
    - "What step am I on?" → CurrentStepHandler (vision-based)
    - "How do I...?" → HelpHandler (RAG-based)

    Args:
        query: User's text query
        manual_id: Manual identifier
        current_step: Optional current step number
        session_id: Optional session ID from previous image upload
        image: Optional image file upload

    Returns:
        QueryResponse with answer, confidence, and metadata
    ...
    # With session ID (from previous upload)
    POST /api/v2/query
    Form data:
    ...
    POST /api/v2/query
    Form data:
        query: "How do I attach the wheels?"
        manual_id: "6454922"
    ```
    """
    from .query.query_router import get_query_router

    try:
        # Get query router instance
        query_router = get_query_router(graph_manager, rag_pipeline)
        ...
        if image:
            ...
            temp_dir = Path(f"/tmp/lego_assembly_temp")
            temp_dir.mkdir(parents=True, exist_ok=True)
            temp_file = temp_dir / f"{uuid.uuid4()}_{image.filename}"
        ...
        # Or use session ID images
        elif session_id:
            upload_dir = Path(f"/tmp/lego_assembly_uploads/{session_id}")
            if upload_dir.exists():
                user_images = sorted([str(p) for p in upload_dir.glob("image_*.*")])
                if user_images:
                    image_path = user_images[0]  # Use first image
                    logger.info(f"Using session image: {image_path}")
        ...
        # Route query
        logger.info(f"Routing query: '{query}' for manual {manual_id}")
        response = query_router.route_query(
            user_query=query,
            manual_id=manual_id,
